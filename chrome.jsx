...
export function ArcadeBar({ round, credits, highScore }) {
  ...
  const t = time.toISOString().substr(11, 8);
  return (
    <div className="arcade-bar">
      ...
      <div className="grp">
        ...
        <span className="label">SYS</span><span style={{ color: "var(--c2-bright)" }}>{t} UTC</span>
      </div>
    </div>
  );
}

export function VSBanner({ power }) {
  ...
  return (
    <div className="vs-banner">
      <div className="vs-side">
        <span className="vs-tag c1">PLAYER 1 · SAFETY-FIRST</span>
        <span className="vs-name c1">CLAUDE</span>
        <span className="vs-org">ANTHROPIC · SAN FRANCISCO</span>
        <span className="vs-stance">stance · constitutional · enterprise-favored</span>
      ...
      <div className="vs-side right">
        <span className="vs-tag c2">PLAYER 2 · DISTRIBUTION-FIRST</span>
        <span className="vs-name c2">CODEX</span>
        <span className="vs-org">OPENAI · SAN FRANCISCO</span>
        <span className="vs-stance">stance · scale · consumer-favored</span>
        <div className="vs-meters" style={{ gridTemplateColumns: "1fr", marginTop: 18 }}>
          {rows.map((r, i) => {
            const max = r.scale || 100;
            return (
...
export function PowerStats({ power }) {
  ...
  const items = [
    { label: "MOMENTUM Δ 7D", v1: power.c1.momentum, v2: power.c2.momentum, unit: "", trendC1: trends.c1?.momentum || "+0", trendC2: trends.c2?.momentum || "+0" },
    { label: "MINDSHARE %",   v1: power.c1.mindshare, v2: power.c2.mindshare, unit: "%", trendC1: trends.c1?.mindshare || "+0", trendC2: trends.c2?.mindshare || "+0" },
    { label: "ENT WINS QTD",  v1: power.c1.enterprise, v2: power.c2.enterprise, unit: "", trendC1: trends.c1?.enterprise || "+0", trendC2: trends.c2?.enterprise || "+0" },
    { label: "SHIPS / MO",    v1: power.c1.ship, v2: power.c2.ship, unit: "", trendC1: trends.c1?.ship || "+0", trendC2: trends.c2?.ship || "+0" },
  ];
  return (
